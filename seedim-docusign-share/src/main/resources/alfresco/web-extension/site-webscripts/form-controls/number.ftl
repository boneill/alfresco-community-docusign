...
<script>
 function invoiceValueOnKeyDown(){
  var otherInput =  YAHOO.util.Selector.query('input[id*=prop_${field.control.params.otherInput}');
  
  <span class="incomplete-warning">${msg(otherInput)}<span> 
 
  if(Number(event.target.value) > otherInput[0].value)
  {
    <span class="incomplete-warning">Expire Warning Days cannot be greater than Days to Expire<span>  
