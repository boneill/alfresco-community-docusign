package au.com.seedim.utils.form.processor;
...
public class SeedimActionFormProcessor extends FilteredFormProcessor<ActionDefinition, ActionFormResult>
{
    public static final String ITEM_KIND = "seedimaction";
    
    protected static final String EXECUTE_ASYNCHRONOUSLY = "executeAsynchronously";
    
    private static Log logger = LogFactory.getLog(SeedimActionFormProcessor.class);
    ...
    protected ActionFormResult internalPersist(ActionDefinition item, FormData data)
    {
        ...
        if (logger.isDebugEnabled()) 
            logger.debug("Execture action: data" + data);
    ...
    private Action createActionAndParams(ActionDefinition item, FormData data)
    {
        ...
        for (ParameterDefinition actionParamDef : actionParamDefs)
        {
            ...
            if (fieldData == null)
            {
                fieldData = data.getFieldData(FormFieldConstants.PROP_DATA_PREFIX +
                                              paramDefinitionName);
            }
            ...
            if (fieldData != null)
            {
                ...
                if (logger.isDebugEnabled()) 
                    logger.debug("fieldData: " + fieldData.getValue());
                ...
                if (actionParamDef.isMultiValued())
                {
                    ...
                    {
                    	if(typeDef.equals(dictionaryService.getDataType(DataTypeDefinition.ASSOC_REF))) {
                        	
                        	typeDef = dictionaryService.getDataType(DataTypeDefinition.TEXT);
                        }
